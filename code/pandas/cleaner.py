...
def main():
    ...
    # Get file names
    for yr in year:
        filenames[yr] = get_file_names(
            bucket_name="emr-output-bucket-unh", prefix="walmart_parquet_" + yr
        )
    ...
    for yr in year:
        data = [
            pd.read_parquet("s3://athena-east-1-satish/" + f) for f in filenames[yr]
        ]
